(function () {
  ...
  function findWorkingPort(ctx, discovery, ideName) {
    ...
    for (var i = 0; i < ports.length; i++) {
      var port = ports[i]
      if (probePort(ctx, "https", port, discovery.csrf, ideName)) return { port: port, scheme: "https" }
      if (probePort(ctx, "http", port, discovery.csrf, ideName)) return { port: port, scheme: "http" }
