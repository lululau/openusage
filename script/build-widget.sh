...
# account required). `script/embed-widget.sh` re-signs the .appex into the host
# app with an ad-hoc (local) or Developer ID (release) identity.
#
# Optional automatic signing (needs Xcode signed-in to the team):
#   DEVELOPMENT_TEAM=XXXXXXXXXX WIDGET_CODE_SIGN=auto script/build-widget.sh
set -euo pipefail
...
echo "  Next: script/build_and_run.sh (dev) or script/release.sh (embeds + signs it)"
